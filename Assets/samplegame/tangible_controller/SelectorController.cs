...
public class SelectorController : MonoBehaviour, Tangible.Controller {
	...
	private Tangible.Controller controller = null;
    ...
    void Tangible.Controller.Mute(bool mute) {
        // Use Muted instead
    ...
    private void OnMuteChanged() {
        ...
        if (physical_controller_ != null) (physical_controller_ as Tangible.Controller).Mute(muted_);
        if (on_screen_controller_ != null) (on_screen_controller_ as Tangible.Controller).Mute(muted_);
    }
		...
		switch(_source) {
			...
			case Source.PHYSICAL_RECORDED:
			{
				if (physical_controller_ == null) {
                    ...
                    (physical_controller_ as Tangible.Controller).Mute(Muted);
				}
				controller = physical_controller_ as Tangible.Controller;
			...
			default: 
			{
				if (on_screen_controller_ == null) {
                    ...
                    (on_screen_controller_ as Tangible.Controller).Mute(Muted);
				}
				controller = on_screen_controller_ as Tangible.Controller;
			} break;
		}
	...
	public bool Register(int _userId, Tangible.EventDelegate _processEvent, int automaticDumpInterval = 0) {
		CreateDeck ();
		return (this as Tangible.Controller).Register (_userId, new Tangible.Config (deck_.GetRecognitionMode ()), _processEvent, automaticDumpInterval);
	}
	...
	bool Tangible.Controller.Register(int _userId, Tangible.Config _config, Tangible.EventDelegate _processEvent, int automaticDumpInterval) {
		(this as Tangible.Controller).Unregister(userId);
		userId = _userId;
		config = _config;
